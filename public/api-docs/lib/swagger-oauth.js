...
function handleLogin() {
  ...
  for (i = 0; i < scopes.length; i++) {
    scope = scopes[i];
    str =
      `<li><input type="checkbox" id="scope_${i}" scope="${scope.scope}"` +
      `" oauthtype="${scope.OAuthSchemeKey}"/>` +
      `<label for="scope_${i}">${scope.scope}`;
    if (scope.description) {
      if (
        $.map(auths, function(n, i) {
          return i;
        }).length > 1
      )
        // if we have more than one scheme, display schemes
        str +=
          `<br/><span class="api-scope-desc">${scope.description} (${
  ...
  popupDialog.find('button.api-popup-authbtn').click(function() {
    ...
    const defaultRedirectUrl = `${host.protocol}//${
      host.host
    }${pathname}/o2c.html`;
    ...
    for (var key in authSchemes) {
      if (
        authSchemes.hasOwnProperty(key) &&
        OAuthSchemeKeys.indexOf(key) != -1
      ) {
        // only look at keys that match this scope.
